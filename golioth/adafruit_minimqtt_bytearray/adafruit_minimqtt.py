...
class MQTT:
    ...
    def _get_connect_socket(self, host, port, *, timeout=1):
        ...
        if self.logger and port == MQTT_TLS_PORT:
            self.logger.info(
                "Establishing a SECURE SSL connection to {0}:{1}".format(
                    host, port)
            )
        elif self.logger:
            self.logger.info(
                "Establishing an INSECURE connection to {0}:{1}".format(
                    host, port)
            )
    ...
    def connect(self, clean_session=True, host=None, port=None, keep_alive=None):
        ...
        if self._lw_topic:
            ...
            var_header[6] |= 0x4 | (self._lw_qos & 0x1) << 3 | (
                self._lw_qos & 0x2) << 3
    ...
    def publish(self, topic, msg, retain=False, qos=0):
        ...
        if len(msg) > MQTT_MSG_MAX_SZ:
            raise MMQTTException(
                "Message size larger than %d bytes." % MQTT_MSG_MAX_SZ)
        assert (
            0 <= qos <= 1
                ...
                if op == 0x40:
                    ...
                    if self._pid == rcv_pid:
                        if self.on_publish is not None:
                            self.on_publish(
                                self, self._user_data, topic, rcv_pid)
                        return
            ...
            if op == 176:
                ...
                for t in topics:
                    if self.on_unsubscribe is not None:
                        self.on_unsubscribe(
                            self, self._user_data, t, self._pid)
    ...
    def disable_logger(self):
        ...
        self.logger = None  # Write your code here :-)
